                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr v-for="u in users" :key="u.id" class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3"><span
                                        class="bg-slate-100 px-2 py-1 rounded text-xs font-medium">{{
                                            u.role }}</span></td>
        ...
        <div v-if="showUserModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-white rounded-xl shadow-2xl w-96 p-6">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button @click="showUserModal = false" class="text-slate-400 hover:text-slate-600"><i
                ...
                <form @submit.prevent="submitUserForm" class="space-y-4">
                    ...
                    <div class="flex items-center gap-2"><input type="checkbox" id="active"
                            v-model="userForm.active"><label htmlFor="active" class="text-sm">Đang hoạt động</label>
                    </div>
                    <div class="pt-4 flex justify-end gap-2">
                        <button type="button" @click="showUserModal = false"
                            class="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded">Hủy</button>
                        <button type="submit"
                            class="px-4 py-2 bg-blue-600 text-white font-bold rounded hover:bg-blue-700">Lưu
                            lại</button>
                    </div>
                </form>
            </div>
...
export default defineComponent({
  ...
  methods: {
    openAddUser() {
      ...
      this.userForm = { id: 0, username: "", name: "", email: "", role: "Viewer", active: true };
    ...
    deleteUser(id: number) {
      ...
      this.logs.unshift({ id: Date.now(), type: "DELETE", message: `Xóa user #${id}`, time: new Date().toISOString() });
    },
    submitUserForm() {
      if (this.editingUser) {
        const idx = this.users.findIndex(u => u.id === this.editingUser?.id);
        if (idx !== -1) this.users[idx] = { ...this.userForm };
        this.logs.unshift({ id: Date.now(), type: "UPDATE", message: `Cập nhật user #${this.userForm.id}`, time: new Date().toISOString() });
      } else {
