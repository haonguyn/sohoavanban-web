            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div class="lg:col-span-5 space-y-6">
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-md border border-gray-200">
                        <h2 class="text-lg font-bold text-gray-800 mb-4 flex items-center">
                            <svg class="w-5 h-5 mr-2 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
                            1. Tải lên tệp nguồn
                        </h2>
                        ...
                        <div @dragover.prevent="onDragOver" @dragleave.prevent="onDragLeave" @drop.prevent="onDrop"
                            ...
                            <div v-if="!selectedFile" class="text-center">
                                <svg class="mx-auto h-10 w-10 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
                            ...
                            <div v-else class="w-full h-full flex flex-col justify-center items-center z-10">
                                ...
                                <button @click="clearFile" class="mt-2 text-xs text-red-500 hover:text-red-700 underline">
                                    Xóa và chọn lại
                                </button>
                            </div>
                        </div>
                        ...
                        <button @click="startOCR" :disabled="!selectedFile || isLoading"
                            class="w-full mt-4 py-2.5 px-4 rounded-lg shadow-sm text-base font-semibold text-white transition-all
                            ...
                            <span v-else class="flex items-center">
                                <svg class="animate-spin h-5 w-5 mr-2" viewBox="0 0 24 24" fill="none"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg>
                                Đang xử lý AI...
                            </span>
                        </button>
                    </div>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-md border border-gray-200 relative">
                        ...
                        <div v-if="isLoading" class="absolute inset-0 bg-white/60 z-10 flex items-center justify-center rounded-xl backdrop-blur-[1px]">
                            <div class="text-center">
                                <span class="text-sm font-semibold text-blue-600 animate-pulse">Đang điền thông tin tự động...</span>
                            </div>
                        </div>

                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-lg font-bold text-gray-800 flex items-center">
                                <svg class="w-5 h-5 mr-2 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                2. Thông tin thuộc tính
                            </h2>
                            <span v-if="isOcrDone" class="text-xs font-semibold text-green-600 bg-green-50 px-2 py-1 rounded-md border border-green-200">
                                ✨ Đã tự động điền
                            </span>
                        </div>
                        ...
                        <form @submit.prevent="saveDocument" class="space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Tên văn bản / Trích yếu <span class="text-red-500">*</span></label>
                                <textarea v-model="form.tenVanBan" rows="2" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="VD: Quyết định về việc..."></textarea>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" v-model="form.soKyHieu" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="123/QĐ-..." />
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Loại văn bản</label>
                                    <select v-model="form.loaiVanBan" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" v-model="form.coQuanBanHanh" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Người ký</label>
                                    <input type="text" v-model="form.nguoiKy" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" v-model="form.soTrang" min="1" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Lĩnh vực</label>
                                    <input type="text" v-model="form.linhVuc" placeholder="VD: Y tế, Giáo dục" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-gray-50 p-3 rounded-lg border border-gray-200">
                                <div>
                                    ...
                                    <input type="date" v-model="form.ngayBanHanh" class="block w-full px-2 py-1.5 border border-gray-300 rounded text-sm" />
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-blue-600 mb-1">Ngày hiệu lực</label>
                                    <input type="date" v-model="form.ngayHieuLuc" class="block w-full px-2 py-1.5 border border-gray-300 rounded text-sm" />
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-red-600 mb-1">Ngày hết hiệu lực</label>
                                    <input type="date" v-model="form.ngayHetHieuLuc" class="block w-full px-2 py-1.5 border border-gray-300 rounded text-sm" />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" v-model="form.ghiChu" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                            </div>
                ...
                <div class="lg:col-span-7 bg-white p-6 rounded-xl shadow-lg border border-gray-200 flex flex-col h-[800px] lg:h-auto lg:min-h-[800px]">
                    <div class="flex justify-between items-center mb-4 border-b border-gray-100 pb-2">
                        ...
                        <div class="flex space-x-2">
                             <button @click="copyText(extractedText)" :disabled="!extractedText"
                                ...
                                <svg class="w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h12a2 2 0 002-2v-1m-4-4l-4 4m0 0l-4-4m4 4V7" /></svg> Copy
                            </button>
                    ...
                    <div class="flex-1 min-h-0 relative bg-gray-50 rounded-lg border border-gray-200 overflow-hidden">
                        ...
                        <textarea v-if="extractedText"
                            v-model="extractedText"
                            placeholder="Văn bản được trích xuất sẽ xuất hiện ở đây..."
                            class="w-full h-full p-6 bg-white font-mono text-sm text-gray-800 focus:outline-none resize-none leading-relaxed"
                        ></textarea>

                        <!-- Trường hợp 2: Đang Loading -->
                        <div v-else-if="isLoading" class="absolute inset-0 flex flex-col items-center justify-center bg-white/90 z-20">
                            <svg class="w-12 h-12 text-blue-600 animate-spin mb-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg>
                        ...
                        <div v-else-if="filePreviewUrl" class="w-full h-full bg-gray-200 flex justify-center items-center overflow-auto">
                            <iframe v-if="fileType === 'pdf'" :src="filePreviewUrl" title="PDF Preview" class="w-full h-full"></iframe> 
                            <img v-else-if="fileType === 'image'" :src="filePreviewUrl" alt="Image Preview" class="max-w-full max-h-full object-contain shadow-lg" />
                        </div>

                        <!-- Trường hợp 4: Chưa có gì -->
                        <div v-else class="absolute inset-0 flex flex-col items-center justify-center text-gray-400">
                             <svg class="w-20 h-20 mb-4 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
