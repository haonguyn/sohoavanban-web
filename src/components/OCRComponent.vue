                ...
                <div class="lg:col-span-5 space-y-6">
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-md border border-gray-200">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <p v-if="isOcrDone" class="text-sm text-gray-500 font-medium">Độ chính xác: {{ accuracy }}%
                            </p>
                        </div>
                        <div @dragover.prevent="onDragOver" @dragleave.prevent="onDragLeave" @drop.prevent="onDrop"
                            ...
                            <div v-else class="w-full h-full flex flex-col justify-center items-center z-10">
                                ...
                                <button @click="clearFile"
                                    class="mt-2 text-xs text-red-500 hover:text-red-700 underline">Xóa và chọn
                                    lại</button>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-lg font-bold text-gray-800 flex items-center">
                                <svg class="w-5 h-5 mr-2 text-green-600" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M9 12h6-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                        ...
                        <form @submit.prevent="saveDocument" @click.stop class="space-y-4">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" v-model="form.page_count" @focus="activeField = 'page_count'"
                                        min="1"
                                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Lĩnh vực</label>
                                    <input type="text" v-model="form.field" @focus="activeField = 'field'"
                                        placeholder="VD: Y tế, Giáo dục"
                                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                                </div>
                            </div>
                            <div
                            ...
                            <div>
                                ...
                                <input type="text" v-model="form.note" @focus="activeField = 'note'"
                                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                            </div>
                ...
                <div
                    ...
                    <div class="p-3 bg-gray-100 flex items-center justify-between border border-gray-200 rounded-t-lg">
                        <div class="flex items-center space-x-4">
                            <span class="text-xs font-bold text-gray-500 uppercase tracking-widest">3. Kết quả trích
                                xuất</span>
                    ...
                    <div
                        ...
                        <div v-show="!isLoading && isOcrDone && !showLayout && positions.length > 0"
                            class="ocr-document-container w-full h-full bg-gray-200 overflow-auto p-1 sm:p-2 absolute inset-0">
                        ...
                        <!-- VIEW 5: Empty -->
                        <div v-if="!isLoading && !filePreviewUrl"
